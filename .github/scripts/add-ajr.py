...
# Compress the original snk loop so there is enough time for the custom finale.
PHASE = 0.55
...
# The snake finishes the real contribution route, comes back from the right,
# then stops here. s0 is the head; the other segments remain 16px behind it.
STOP_X = 624
STOP_Y = 48

# Finale timing (% of the complete loop).
...
def letter_targets():
    """AJR made from the same 16px grid as GitHub and centered in the calendar."""
    letter_cols = 5
    gap_cols = 2
    ...
    for ch in "AJR":
        ...
        for row, line in enumerate(FONT[ch]):
            for col, bit in enumerate(line):
                ...
                # x/y match the visible contribution squares exactly.
...
def active_cells(svg):
    """Collect every real non-empty contribution cell generated by snk."""
    cells = []
    pattern = re.compile(
...
def build_complete_grid():
    """A full 53x7 gray calendar so there are never invisible/missing slots."""
...
def build_final_clear(cells):
    """Hard guarantee: no real green contribution can survive into the AJR phase."""
...
def latest_contribution_consumption(style):
    """Return the latest percentage at which a real contribution becomes empty."""
    values = []
...
def find_return_crossing(frames, stop_x):
    """Find the right-to-left crossing after the snake has reached the far right."""
    seen_right = False
    prev = None
    ...
    raise RuntimeError(f"Could not find return crossing at x={stop_x}")


def nearest_route(groups):
    """Eat R, then J, then A using a short nearest-neighbour route inside each letter."""
    route = []
    ...
    for group in reversed(groups):
        ...
        while remaining:
            def distance(item):
                _, x, y = item
                # Convert visible square x/y to the snake transform coordinate system.
                tx, ty = x - 2, y - 2
...
def route_timing(route):
    """Map every AJR block to the exact time the snake head reaches it."""
    if not route:
        return {}, []
    ...
    for i, (idx, x, y) in enumerate(route):
        if n == 1:
            ...
            pct = EAT_START + (EAT_END - EAT_START) * (i / (n - 1))
        times[idx] = pct
...
def build_dot_css(ms, groups, eat_times):
    """One animation per block: spit -> settle -> wait -> disappear under the snake head."""
    mouth_x = STOP_X + 2
    mouth_y = STOP_Y + 2
    css = [
    ...
    for letter_index, pts in enumerate(groups):
        ...
        for local_i, (idx, tx, ty) in enumerate(pts):
            ...
            # Small, natural puff from the mouth. No explosion/fan effect.
...
def rewrite_snake_keyframes(style, timed_route, latest_consumed):
    """Finish real stats, stop coherently, eat AJR, then travel back left before reset."""
    ...
    # Head route after it starts eating. Add a short exit to the left at the end.
    ...
    for idx, stop_x in enumerate(stop_positions):
        ...
        crossing, _ = find_return_crossing(frames, stop_x)
        stop_times.append(crossing)

        if crossing <= latest_consumed:
            raise RuntimeError(
                f"Snake segment s{idx} would stop at {crossing:.2f}% before all contributions are consumed ({latest_consumed:.2f}%)"
            )
        ...
        # Each body segment follows the exact head route with a small positional lag.
        ...
        # Return left naturally after the last letter has been eaten.
        ...
        for j, (px, py) in enumerate(exit_points):
            ...
            # Keep the body trailing the head during the exit.
            lag_px = px + idx * 16
            new_parts.append(f"{pct:.2f}%{{transform:translate({lag_px}px,{py}px)}}")
...
def clip_snake(svg):
    """The snake can never render outside the 53x7 contribution calendar."""
    clip = (
...
def validate(svg, active_count, groups):
    ...
    # Centering guard: the average AJR x coordinate must match the grid center closely.
...
def patch(path):
    ...
    # Compress the entire original snk animation into the first part of the loop.
    ...
    # Full gray grid goes behind the real contribution cells.
    ...
    # Forced consumed state goes above real contributions but below the snake/AJR.
